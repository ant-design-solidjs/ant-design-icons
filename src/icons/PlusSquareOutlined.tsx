/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import PlusSquareOutlinedSvg from '@ant-design/icons-svg/lib/asn/PlusSquareOutlined';
import AntdIcon from '../components/AntdIcon'

function PlusSquareOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={PlusSquareOutlinedSvg} />
}

 /**![plus-square](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = PlusSquareOutlined

export default RefIcon;