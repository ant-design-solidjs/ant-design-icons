/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import CopyrightCircleFilledSvg from '@ant-design/icons-svg/lib/asn/CopyrightCircleFilled';
import AntdIcon from '../components/AntdIcon'

function CopyrightCircleFilled(props: AntdIconProps) {
    return <AntdIcon {...props} icon={CopyrightCircleFilledSvg} />
}

 /**![copyright-circle](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = CopyrightCircleFilled

export default RefIcon;