/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import PercentageOutlinedSvg from '@ant-design/icons-svg/lib/asn/PercentageOutlined';
import AntdIcon from '../components/AntdIcon'

function PercentageOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={PercentageOutlinedSvg} />
}

 /**![percentage](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = PercentageOutlined

export default RefIcon;