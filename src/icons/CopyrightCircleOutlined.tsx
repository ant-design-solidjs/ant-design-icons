/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import CopyrightCircleOutlinedSvg from '@ant-design/icons-svg/lib/asn/CopyrightCircleOutlined';
import AntdIcon from '../components/AntdIcon'

function CopyrightCircleOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={CopyrightCircleOutlinedSvg} />
}

 /**![copyright-circle](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = CopyrightCircleOutlined

export default RefIcon;