/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import PlayCircleOutlinedSvg from '@ant-design/icons-svg/lib/asn/PlayCircleOutlined';
import AntdIcon from '../components/AntdIcon'

function PlayCircleOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={PlayCircleOutlinedSvg} />
}

 /**![play-circle](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = PlayCircleOutlined

export default RefIcon;