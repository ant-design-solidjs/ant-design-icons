/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import NotificationTwoToneSvg from '@ant-design/icons-svg/lib/asn/NotificationTwoTone';
import AntdIcon from '../components/AntdIcon'

function NotificationTwoTone(props: AntdIconProps) {
    return <AntdIcon {...props} icon={NotificationTwoToneSvg} />
}

 /**![notification](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = NotificationTwoTone

export default RefIcon;