/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import LeftCircleOutlinedSvg from '@ant-design/icons-svg/lib/asn/LeftCircleOutlined';
import AntdIcon from '../components/AntdIcon'

function LeftCircleOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={LeftCircleOutlinedSvg} />
}

 /**![left-circle](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = LeftCircleOutlined

export default RefIcon;