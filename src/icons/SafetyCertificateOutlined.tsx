/* eslint-disable eslint-comments/no-unlimited-disable */
/* eslint-disable */
// GENERATE BY ./scripts/generate.ts
// DON NOT EDIT IT MANUALLY

import type { Component } from 'solid-js'
import type { AntdIconProps } from '../components/AntdIcon'
import SafetyCertificateOutlinedSvg from '@ant-design/icons-svg/lib/asn/SafetyCertificateOutlined';
import AntdIcon from '../components/AntdIcon'

function SafetyCertificateOutlined(props: AntdIconProps) {
    return <AntdIcon {...props} icon={SafetyCertificateOutlinedSvg} />
}

 /**![safety-certificate](data:image/svg+xml;base64,) */ 
const RefIcon: Component<AntdIconProps> = SafetyCertificateOutlined

export default RefIcon;